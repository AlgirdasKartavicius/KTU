...
function L2_1_3
    ...
    axis([min(SX)-5 max(SX)+5 min(SY)-5 max(SY)+5]);
    ts=interp1(SX,SY,t);
    clear SX SY, SX=t;SY=ts;plot(SX,SY,'r.');   
    title(sprintf('Duota funkcija, taðkø skaièius 2^n, kur n = %d',n));
    xmin=min(SX);xmax=max(SX);
    ymin=min(SY);ymax=max(SY);


    % Aproksimavimas Haro bangelemis:
    m=10  % detalumo lygiu skaicius
    ...
    figure(2);subplot(7,1,1),axis equal,axis([xmin-5 xmax+5 ymin-5 ymax+5]); hold on,grid on, plot(SX,h,'Linewidth',2);title(sprintf('lygyje %d aproksimuotas signalas',0));

    for i=0:5 %detalumo didinimo ciklas
        % apskaiciuojamos funkcijos detales:  
        h1=zeros(1,nnn); for k=0:2^(n-m+i)-1, h1=h1+details{m-i}(k+1)*Haar_wavelet(SX,n-m+i,k,a,b);  end
        figure(3),subplot(6,1,i+1), axis equal,hold on,grid on
        ...
        figure(2);subplot(7,1,i+2),axis equal,axis([xmin-5 xmax+5 ymin-5 ymax+5]), hold on,grid on, plot(SX,h,'Linewidth',2);title(sprintf('lygyje %d aproksimuotas signalas' ,i+1));
    end
