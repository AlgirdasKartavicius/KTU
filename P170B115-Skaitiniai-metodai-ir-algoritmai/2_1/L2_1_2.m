...
clc,close all
hL=[];
raide = 0; % 0 - raidë M, 1 - raidë K
% X=[-1  0.5  1  1  -1 -1 ]
% Y=[ 1  0.5  1 -1  -1  1 ]

if raide == 0
    X=[-1 -0.4 2.3  1.9 4.2  2.9 3.9] 
    Y=[-2 -2.9 3.7 -1.3 3.6 -2.9  -2]
else
    X=[-1.3 -1.2 -0.3    1  1.5 0.8 2.9 1.5 -0.2 -0.2 -1.3] 
    Y=[   3 -2.7  0.1 -2.9 -2.9 0.8   3   3  1.3  3      3]
end
...
f=figure(1);axis([min(X)-3*dist,max(X)+3*dist,min(Y)-3*dist,max(Y)+3*dist]);axis equal;hold on; grid on;

% vaizduojame duotus taskus
for i=1:nP, 
    h(i)=plot(X(i), Y(i),'ko','ButtonDownFcn',@startDragFcn,'MarkerSize',15);
