...
fclose all; fhx=fopen('autox.txt','r'); fhy=fopen('autoy.txt','r');
...
%m=6  % detalumo lygiu skaicius   n >= m
...
figure(30);axis equal,axis([xmin xmax ymin ymax]); hold on,grid on, plot(SX,h,'Linewidth',2);title(sprintf('lygyje %d aproksimuota funkcija',0));

                        % subplot sudeda á vienà
...
for i=0:m-1 %detalumo didinimo ciklas
    ...
    figure(i+20); axis equal,hold on,grid on
    ...
    figure(i+10);axis equal,axis([xmin xmax ymin ymax]), hold on,grid on, plot(SX,h,'Linewidth',2);title(sprintf('lygyje %d aproksimuota funkcija' ,i+1));
end
