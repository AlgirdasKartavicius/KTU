...
class contracts {
	...
	public function getSumPriceOfPrekes($kodas){
		$whereClauseString = " WHERE `Preke`.`kodas`='{$kodas}'";
		$query = " SELECT SUM(`Preke`.`kaina`) as `kaina`";
		$data = mysql::select($query);
		return $data;
	}
	
	public function getSportoSakosPrekes($id) {		
		$query = "	SELECT 	`Preke`.`kodas`,
							`Preke`.`pavadinimas` as `prekes_pavadinimas`,
							`Preke`.`kaina` as `prekes_kaina`,
							`SportoSakosIrankioKategorija`.`pavadinimas` as `kategorija`
					FROM `Preke`, `SportoSakosIrankioKategorija`, `SportoSaka`
					WHERE `Preke`.`fk_SportoSakosIrankioKategorijaid` = `SportoSakosIrankioKategorija`.`id`
					AND `SportoSakosIrankioKategorija`.`fk_SportoSakaid` = `SportoSaka`.`id`
					AND `SportoSaka`.`id` = '{$id}'
					ORDER BY `SportoSakosIrankioKategorija`.`pavadinimas` ASC, `prekes_pavadinimas` ASC";
	...
	public function getSportoSakosKategorijosPrekiuSum($id, $kategorija){
		$query = "	SELECT 	`SportoSakosIrankioKategorija`.`pavadinimas` as `kategorija`,
							ROUND(SUM(`Preke`.`kaina`), 2) as `kaina`
					FROM `Preke`, `SportoSakosIrankioKategorija`, `SportoSaka`
					WHERE `Preke`.`fk_SportoSakosIrankioKategorijaid` = `SportoSakosIrankioKategorija`.`id`
					AND `SportoSakosIrankioKategorija`.`fk_SportoSakaid` = `SportoSaka`.`id`
					AND `SportoSakosIrankioKategorija`.`pavadinimas` = '{$kategorija}'
					AND `SportoSaka`.`id` = '{$id}'";
	...
	public function getMaxPreke($id){
		$query = "	SELECT 	`SportoSakosIrankioKategorija`.`pavadinimas` as `kategorija`,
							MAX(`Preke`.`kaina`) as `max_kaina`
					FROM `Preke`, `SportoSakosIrankioKategorija`, `SportoSaka`
					WHERE `Preke`.`fk_SportoSakosIrankioKategorijaid` = `SportoSakosIrankioKategorija`.`id`
					AND `SportoSakosIrankioKategorija`.`fk_SportoSakaid` = `SportoSaka`.`id`
					AND `SportoSaka`.`id` = '{$id}'";
		...
		return $data[0]['max_kaina'];
	}
	
	public function getPrekesSum($id){
		$query = "	SELECT 	`SportoSakosIrankioKategorija`.`pavadinimas` as `kategorija`,
							ROUND(SUM(`Preke`.`kaina`), 2) as `bendra_suma`
					FROM `Preke`, `SportoSakosIrankioKategorija`, `SportoSaka`
					WHERE `Preke`.`fk_SportoSakosIrankioKategorijaid` = `SportoSakosIrankioKategorija`.`id`
					AND `SportoSakosIrankioKategorija`.`fk_SportoSakaid` = `SportoSaka`.`id`
					AND `SportoSaka`.`id` = '{$id}'";
		...
		return $data[0]['bendra_suma'];
	}
	
	public function getCustomerOrders($dateFrom, $dateTo) {
		$whereClauseString = "";
		if(!empty($dateFrom)) {
		...
		$query = "  SELECT  `Saskaita`.`nr`,
							...
							`Pirkejas`.`vardas`,
						    `Pirkejas`.`pavarde`,
						    `Saskaita`.`suma` as `saskaitos_kaina`,
							`AptarnaujantisAsistentas`.`tabelioNr`,
						    `t`.`bendra_kliento_saskaitu_suma`,
							`s`.`bendras_kliento_saskaitu_skaicius`
					FROM `Saskaita`
						INNER JOIN `Pirkejas`
							ON `Saskaita`.`fk_Pirkejaskodas`=`Pirkejas`.`kodas`
						INNER JOIN `AptarnaujantisAsistentas`
							ON `Saskaita`.`fk_AptarnaujantisAsistentastabelioNr`=`AptarnaujantisAsistentas`.`tabelioNr`
						LEFT JOIN (
							SELECT `kodas`,
									sum(`Saskaita`.`suma`) AS `bendra_kliento_saskaitu_suma`
							FROM `Saskaita`
								INNER JOIN `Pirkejas`
									ON `Saskaita`.`fk_Pirkejaskodas`=`Pirkejas`.`kodas`
							{$whereClauseString}
							GROUP BY `kodas`
						) `t` ON `t`.`kodas`=`Pirkejas`.`kodas`
						LEFT JOIN (
							SELECT `kodas`,
									COUNT(`Saskaita`.`nr`) AS `bendras_kliento_saskaitu_skaicius`
							FROM `Saskaita`
								INNER JOIN `Pirkejas`
									ON `Saskaita`.`fk_Pirkejaskodas`=`Pirkejas`.`kodas`
							{$whereClauseString}
							GROUP BY `kodas`
						) `s` ON `s`.`kodas`=`Pirkejas`.`kodas`
					{$whereClauseString}
					GROUP BY `Saskaita`.`nr` ORDER BY `Pirkejas`.`vardas` ASC, `Pirkejas`.`pavarde` ASC";
	...
	public function getSumPriceOfOrders($dateFrom, $dateTo) {
		...
		$query = "  SELECT sum(`Saskaita`.`suma`) AS `saskaitos_suma`
					FROM `Saskaita`
					{$whereClauseString}";
		$data = mysql::select($query);
	...
	public function getCountOfOrders($dateFrom, $dateTo) {
		$whereClauseString = "";
		if(!empty($dateFrom)) {
			$whereClauseString .= " WHERE `Saskaita`.`data`>='{$dateFrom}'";
			if(!empty($dateTo)) {
		...
		$query = "  SELECT COUNT(`Saskaita`.`nr`) AS `saskaitu_kiekis`
					FROM `Saskaita`
					{$whereClauseString}";
		$data = mysql::select($query);
	...
	public function getPrekesPriceLowerThan($kaina) {
		$whereClauseString = "";
		if(!empty($kaina)) {
			$whereClauseString .= " AND `Preke`.`kaina`<'{$kaina}'";
		}
		$query = "  SELECT `Preke`.`kodas`,
						   `Preke`.`pavadinimas`,
						   `SportoSakosIrankioKategorija`.`pavadinimas` as `kategorija`,
						   `Preke`.`kaina`
					FROM `Preke`, `SportoSakosIrankioKategorija`
					WHERE `SportoSakosIrankioKategorija`.`id`=`Preke`.`fk_SportoSakosIrankioKategorijaid`
					{$whereClauseString}
					ORDER BY `kategorija` ASC, `Preke`.`kaina` ASC";
		$data = mysql::select($query);
