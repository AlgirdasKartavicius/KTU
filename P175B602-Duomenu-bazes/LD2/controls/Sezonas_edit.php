	...
	if(!empty($_POST['submit'])) {
		...
		if($validator->validate($_POST)) {
			...
			if(isset($data['id'])) {
				...
				$sezonasObj->updateSezonas($data);
			} else {
				// randame didžiausią sezono id duomenų bazėje
				$latestId = $sezonasObj->getMaxIdOfSezonas();
				
				// įrašome naują įrašą
				$data['id'] = $latestId + 1;
				$sezonasObj->insertSezonas($data);
			}
		...
		if(!empty($id)) {
			$fields = $sezonasObj->getSezonas($id);
		}
	}
?>
<ul id="pagePath">
	<li><a href="index.php">Pradžia</a></li>
	<li><a href="index.php?module=<?php echo $module; ?>">Sezonai</a></li>
	<li><?php if(!empty($id)) echo "Sezono redagavimas"; else echo "Naujas sezonas"; ?></li>
</ul>
...
<div id="formContainer">
	...
	<form action="" method="post">
		<fieldset>
			<legend>Sezono informacija</legend>
			<p>
