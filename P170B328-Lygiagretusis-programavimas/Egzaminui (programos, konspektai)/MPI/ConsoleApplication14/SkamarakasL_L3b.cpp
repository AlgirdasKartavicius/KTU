...
const char duomenuFailas[] = "SkamarakasL_L3_1.txt";		// Duomenu failo vardas
...
int main(int argc, char *argv[])
{
	...
	cout << "PROCESS EXIT NR: " << world_rank << endl;
	...
	MPI_Request flag;
	MPI_Irecv(&rasymasbaigesi, 1, MPI_INT, 0, RASYMO_PABAIGA_TAG, pagr_komunikatorius, &flag);
	while(V->kiekis > 0){
		...
		for(int i=0; i<V->kiekis; i++)
		{
			...
			strcpy(gijuMasyvoElementas.vardas, vardas.vardas);
	...
	while(F->studentuSkaicius > 0) {
		...
		strcpy(gijuMasyvoElementas.vardas, studentas.vardas);
	...
	for (int i = 0; i < (RASYMO_PROCESU_SKAICIUS+SKAITYMO_PROCESU_SKAICIUS); i++)
	{
		MPI_Comm_free(&vald_komunikatoriai[i]);
	}
...
}void Skaityti(const char duomenuFailas[], int &fakultetuSkaicius, int &varduMasyvuSkaicius, Vardai vardai[], Fakultetas fakultetai[])
{
	 ...
	 while (!fd.eof()) {
		...
		if(!fakultetuPabaiga){
			strcpy(fakultetas.pavadinimas, vardas.c_str());
			fakultetas.studentuSkaicius = 0;
			for(int i = 0; i < kiekis; i ++){
				fd >> vardas >> kursas >> vidurkis;
				strcpy(studentas.vardas, vardas.c_str());
			...
			for(int i = 0; i < kiekis; i ++){
				...
				strcpy(vardas_obj.vardas, vardas_2.c_str());
...
}int iterptiElemetaBendramGijuMasyve(GijuMasyvoElementas bendrasGijuMasyvas[], GijuMasyvoElementas elementas, int &elementuSkaiciusBendramGijuMasyve)
{
	...
	if(string(bendrasGijuMasyvas[iterpiamoElementoNr].vardas) == string(elementas.vardas)){
		strcpy(bendrasGijuMasyvas[iterpiamoElementoNr].vardas, elementas.vardas);
