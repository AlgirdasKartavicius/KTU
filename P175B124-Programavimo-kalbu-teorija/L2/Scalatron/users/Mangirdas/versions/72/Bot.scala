...
object ControlFunction {
  ...
  def forMaster(bot: Bot) {
    val (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestPredator, nearestPoison) = analyzeViewAsMaster(bot,bot.view)
    bot.log(bot.time.toString())
    ...
    val dontFireBaitUntil              = bot.inputAsIntOrElse("dontFireBaitUntil", -1)
   ...
   // var bfs = BFS(bot, bot.stringView)
   // bfs.fillMatrix();
   ...
   // bfs.bfsPath(480,600);
    ...
    if(dontFirePathFinderUntil < bot.time && bot.energy > 2000  ){
        if(nearestPoison != -1){
            var bfs = BFS(bot, bot.stringView)
            bfs.fillMatrix();
            bfs.findAllNeighbors();
            bfs.bfsPath(480,nearestPoison)
            var moves = bfs.path.toString ;
            bot.log("moves " + moves)
            bot.log("nearestPoison" + nearestPoison)
            bot.spawn(bot.view.center,"mood" -> MiniBot.PathFinder, "path"->moves, "timeT"->bot.time.toString(), "target" -> "", "energy" -> 100)
            bot.set("dontFirePathFinderUntil" -> (bot.time + 100))
        }
    }
    if (dontFireAggressiveMissileUntil < bot.time && bot.energy > 150) {
      // fire attack missile?
      nearestEnemyMaster match {
        ...
        case Some(relPos) => // a master is nearby
            ...
            bot.spawn(unitDelta,
                      "mood" -> MiniBot.AGGRESSIVE,
                      "target" -> remainder,
                      "energy" -> math.max(100, bot.energy * 0.01))
    ...
    if (dontFireGatheringMiniBotUntil < bot.time && bot.energy > 100) {
      bot.spawn(bot.view.center, "mood" -> MiniBot.GATHERING, "target" -> "", "limit" -> 250 * (rand.nextInt(3) + 2))
      bot.set("dontFireGatheringMiniBotUntil" -> (bot.time + rand.nextInt(4)))
    }
    if(dontFireBaitUntil < bot.time && bot.energy > 1000){
        nearestPredator match{
        case None => // no-on nearby
        case Some(relPos) => // a master is nearby
            val unitDelta = relPos.signum
            val remainder = relPos - unitDelta // we place slave nearer target, so subtract that from overall delta
             bot.spawn(unitDelta,
                      ...
                      "target" -> remainder,
                      "energy" -> 100)
            bot.set("dontFireDefensiveAggressiveMissileUntil" -> (bot.time + relPos.stepCount + 10))
        }
  ...
  def forSlave(bot: MiniBot) {
    bot.inputOrElse("mood", "Gathering") match {
      ...
      case MiniBot.BAIT       => reactAsBait(bot)
      case MiniBot.PathFinder => reactPathFinder(bot,bot.inputOrElse("path","NO"),bot.inputOrElse("timeT","NO"))
  ...
  def reactAsAggressiveMissile(bot: MiniBot) {
    bot.view.offsetToNearest(Entities.ENEMY) match {
      case Some(delta: XY) =>
        ...
        if (delta.length <= 2) {        // close enough to blow it up?
          bot.explode(3)                // yes -- blow it up!
        } else {
          bot.move(delta.signum)        // no -- move closer!
      ...
      if (target.isNonZero) {           // did we arrive at the target?
        // no -- keep going
...
def reactAsBait(bot: MiniBot) {
    bot.view.offsetToNearest(Entities.PREDATOR) match {
      case Some(delta: XY) =>

        if (delta.length <= 2) {        // close enough to blow it up?
          bot.explode(9)                // yes -- blow it up!
        } else {
          bot.move(delta.signum)        // no -- move closer!
          bot.set("rx" -> delta.x, "ry" -> delta.y)
        }
  ...
  def reactAsDefensiveMissile(bot: MiniBot) {
    bot.view.offsetToNearest(Entities.ENEMY_MINI_BOT) match {
      ...
      if (target.isNonZero) {           // no -- keep going
        val unitDelta = target.signum // e.g. CellPos(-8,6) => CellPos(-1,1)
        bot.move(unitDelta)
        // compute the remaining delta and encode it into a new 'target' property
        val remainder = target - unitDelta // e.g. = CellPos(-7,5)
        bot.set("target" -> remainder)
      } else { // yes -- but we did not annihilate yet, and are not pursuing anything?!? => switch purpose
        bot.set("mood" -> MiniBot.GATHERING, "target" -> "", "limit" -> 1000)
   ...
   */
  def analyzeViewAsMaster(bot: Bot,view: View) = {
    val directionValue = Array.ofDim[Double](8)
    var nearestEnemyMaster: Option[XY] = None
    var nearestEnemySlave: Option[XY] = None
    var nearestPredator: Option[XY] = None
    var nearestPoison: Int = -1
                                     ...
                                     if (cellRelPos.isNonZero) {
                                       ...
                                       val value: Double = c match {
                                         ...
                                         case Entities.PREDATOR =>
                                           ...
                                           nearestPredator = Some(cellRelPos)
    ...
    (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestPredator, nearestPoison)
  }
                                     ...
                                     if (cellRelPos.isNonZero) {
                                       ...
                                       val value: Double = c match {
                                         ...
                                         case Entities.MASTER =>
                                           // our own master; we're not ready to return
...
trait Bot {
  ...
  def getPath(keyPrefix: String): String
  def view: View
  def energy: Int
  def time: Int
  def generation: Int
  def stringView: String
  def innerNum: Int
  // outputs
...
case class BotImpl(inputParams: Map[String, String]) extends MiniBot {
  ...
  def getPath(key: String) = inputParams.get(key).toString()
  ...
  val stringView = inputParams("view");
...
object Entities {
  val WALL           = 'W' // Wall, you should avoid these
  val MASTER         = 'M' // Bot (=master; yours, always in the center unless seen by a slave)
  val MINI_BOT       = 'S' // Mini-bot (=slave, yours)
  val ENEMY          = 'm' // Bot (=master; enemy, not you)
  val ENEMY_MINI_BOT = 's' // Mini-bot (=slave; enemy's, not yours)
  val FOOD_PLANT     = 'P' // Zugar (=good plant, food)
  val FOOD_BEAST     = 'B' // Fluppet (=good beast, food)
  val POISON         = 'p' // Toxifera (=bad plant, poisonous)
  val PREDATOR       = 'b' // Snorg (=bad beast, predator)
  val UNKNOWN        = '?' // blocked view
  val EMPTY          = '_' // empty block
}
...
case class BFS(bot: Bot ,view: String){
    ...
    def isGood(c: Char) = (c == Entities.EMPTY || c == Entities.FOOD_PLANT  || c == Entities.FOOD_BEAST )
    def fillMatrix(){
        for(i <- 0 to 30 ){
            var str = bot.stringView.substring(i*31, (i+1) * 31);
