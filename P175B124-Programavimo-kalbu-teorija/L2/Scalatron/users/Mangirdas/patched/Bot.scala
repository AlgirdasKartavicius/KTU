package scalatron.botwar.botPlugin.Mangirdas
...
object ControlFunction
{
    ...
    def forMaster(bot: Bot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestPlant, nearestEnemyMasterIndex) = analyzeViewAsMaster(bot.view)
        ...
        // BFS based movement
        if (nearestPlant != -1){
            val bfs = BreadthFirstSearch(bot, bot.viewString)
            bfs.stringViewToMatrix()
            bfs.findAllAdjacentNodes()
            bfs.findPath(480, nearestPlant)
            val bfsPath = bfs.path.toString
            bot.log("bfsPath:\n" + bfsPath)
            val directions = bfsPath.split(";")
            var moves = new ArrayBuffer[XY]()
            for (i <- 0 to directions.length-1) moves += XY(directions(i))
            bot.move(moves(0))
        }
        // Agressive missile
        ...
        // Mines
        if(dontThrowMinesUntil < bot.time && bot.energy > 100 && nearestEnemyMasterIndex != -1){
            val bfsForMines = BreadthFirstSearch(bot, bot.viewString)
            bfsForMines.stringViewToMatrix()
            bfsForMines.findAllAdjacentNodes()
            bfsForMines.findPath(480, nearestEnemyMasterIndex)
            val bfsMinesPath = bfsForMines.path.toString
            val places = bfsMinesPath.split(";")
            var mines = new ArrayBuffer[XY]()
            for (i <- 0 to places.length-1) mines += XY(places(i))
            nearestEnemyMaster match{
                case None => // do nothing (don't throw) 
                case Some(relPos) =>
                    bot.spawn(mines(0), "mood" -> "Mine")
                    bot.set("dontThrowMinesUntil" -> (bot.time + relPos.stepCount + 1))
            }
        }
        bot.move(direction)
    }
    ...
    def analyzeViewAsMaster(view: View) = {
        ...
        var nearestEnemyMasterIndex: Int = -1
        ...
        (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestPlant, nearestEnemyMasterIndex)
    }
...
case class BreadthFirstSearch(bot: Bot, view: String){
    ...
    def findPath(source: Int, destination: Int){
        ...
        var sourceNode = Node(source, XY(0,0))
        ...
        while(!queue.isEmpty){
            var list = adjacentNodes(queue.dequeue.key).toList
            inner.breakable{
                for(ppp <- list){
        ...
        for (vvv <- shortestPath.toArray.reverse) path ++= vvv.value.toString() + ";"
        path.toString
    }
}
